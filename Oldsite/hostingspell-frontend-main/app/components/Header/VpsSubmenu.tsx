...
export interface VpsSubmenuItem {
    key: string;
    title: string;
    desc: string;
    href: string;
...
interface VpsSubmenuProps {
    items: VpsSubmenuItem[];
}

export function VpsSubmenu({ items }: VpsSubmenuProps) {
    const pathname = usePathname();

    return (
        <div className="absolute left-0 top-full w-[900px] bg-white dark:bg-gray-900 rounded-b-xl rounded-tr-xl rounded-tl-none shadow-lg flex gap-4 px-4 py-4 z-30 border-t-0">
            {items.map((item) => {
